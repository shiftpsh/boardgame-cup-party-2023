...
// TODO further optimize snackbar
...
const SnackbarItemContainer = styled.div<SnackbarItemContainerProps>`
  ...
  display: flex;
  padding: 8px;
  background: ${({ theme }) => theme.color.background.card.main};
  border-radius: 4px;
  box-shadow: ${({ themeColor }) => transparentize(0.6, themeColor)} 0px 8px
    16px;
  align-items: center;
  ...
  opacity: ${({ phase }) =>
    phase === "destroying" || phase === "request-creation" ? 0 : 1};
  pointer-events: all;
`;
...
const snackbarIcon = (severity: SnackbarSeverity) => {
  if (severity === "info") return <IconInfoCircle />;
  if (severity === "warn") return <IconAlertTriangle />;
  if (severity === "error") return <IconCircleX />;
  if (severity === "success") return <IconCircleCheck />;
  if (severity === "updating") return <IconRefresh />;
  if (severity === "rare" || severity === "epic" || severity === "legendary") {
    return (
      // eslint-disable-next-line @next/next/no-img-element
      <img
        src={`https://static.solved.ac/tier_small/item-${severity}.svg`}
        style={{ height: "1em" }}
        alt={severity}
      />
    );
...
const useSnackbarColor = (severity: SnackbarSeverity) => {
  ...
  if (severity === "info") return theme.color.text.secondary.main;
  if (severity === "warn") return theme.color.status.warn;
  if (severity === "error") return theme.color.status.error;
  if (severity === "success") return theme.color.status.success;
  if (severity === "rare") return "#0089f9";
  if (severity === "epic") return "#8b00e5";
  if (severity === "legendary") return "#ff7900";
  if (severity === "updating") return theme.color.solvedAc;
...
const SnackbarItemDisplay = React.forwardRef<HTMLDivElement, SnackbarItemProps>(
  (props, ref) => {
      ...
      >
        <SnackbarItemContainer themeColor={color} phase={phase} ref={ref}>
          <SnackbarIconContainer
            themeColor={color}
            rotateAnimation={item.severity === "updating"}
          >
            {icon}
          </SnackbarIconContainer>
...
const SnackbarProviderLayout: React.FC<Props> = (props) => {
    ...
    setDisplayedItems((prevDisplayedItems) =>
      [
        ...items
          .filter(
            (i) =>
              prevDisplayedItems.find((p) => p.item.id === i.id) === undefined
          )
          .map(
            (i) =>
              ({
  ...
  const destroy = useCallback(
    (id: number) => {
      setDisplayedItems((prevDisplayedItems) =>
        prevDisplayedItems.filter((p) => p.item.id !== id)
      );
    },
    [setDisplayedItems]
  );
  ...
  const removeTimeoutByItemId = useCallback(
    (id: number) => {
      setTimeouts((t) => {
        t.filter((tt) => tt.itemId === id && tt.phase !== "destroying").forEach(
          (tt) => {
      ...
      ...displayedItems
        .filter(
          (i) =>
            ...
            !prevTimeouts.find(
              (p) => p.itemId === i.item.id && p.phase === "request-creation"
            )
        )
        .map((i) => {
      ...
      ...displayedItems
        .filter(
          (i) =>
            i.phase === "creating" &&
            !prevTimeouts.find(
              (p) => p.itemId === i.item.id && p.phase === "creating"
            )
        )
        .map((i) => {
          const id: ReturnType<typeof setTimeout> = setTimeout(() => {
      ...
      ...displayedItems
        .filter(
          (i) =>
            ...
            !prevTimeouts.find(
              (p) => p.itemId === i.item.id && p.phase === "created"
            )
        )
        .map((i) => {
          const id: ReturnType<typeof setTimeout> = setTimeout(() => {
      ...
      ...displayedItems
        .filter(
          (i) =>
            i.phase === "destroying" &&
            !prevTimeouts.find(
              (p) => p.itemId === i.item.id && p.phase === "destroying"
            )
        )
        .map((i) => {
          const id: ReturnType<typeof setTimeout> = setTimeout(() => {
  ...
  const invalidateHeights = useCallback(() => {
    setDisplayedItemHeights(
      displayedItems.map((i) => ({
        ...
        height:
          i.phase !== "destroying"
            ? elements.current.get(i.item.id)?.offsetHeight ?? 0
            : -16,
      }))
    );
  ...
  const invalidatePositions = useCallback(() => {
    setDisplayedItemPos(
      displayedItems.map((i) => ({
        id: i.item.id,
        position: displayedItemHeights
          .filter((d) => d.id > i.item.id)
          .reduceRight((a, b) => a + b.height + 16, 16),
      }))
    );
        ...
        {displayedItems.map(({ item, phase }) => {
          ...
          return (
            <SnackbarItemDisplay
              ...
              position={
                displayedItemPos.find((i) => i.id === item.id)?.position ?? 0
              }
            />
          );
        })}
  ...
  return mounted
    ? ReactDOM.createPortal(children, document.body, "snackbar")
    : null;
};
